  ...
  return (
    <section className="relative h-screen flex flex-col" style={{ overflowX: "clip" }}>
      <FadeIn as="nav" delay={0} y={-20} className="flex justify-between items-center px-6 md:px-10 pt-6 md:pt-8 relative z-20">
        {NAV.map((n) => (
          <a key={n} href="#" className="text-[#D7E2EA] font-medium uppercase tracking-wider text-sm md:text-lg lg:text-[1.4rem] transition-opacity duration-200 hover:opacity-70">
            {n}
          </a>
        ))}
      </FadeIn>
      ...
      <div className="flex-1 flex flex-col justify-between relative">
        <FadeIn delay={0.15} y={40} className="overflow-hidden px-6 md:px-10 mt-6 sm:mt-4 md:-mt-5 relative z-0">
